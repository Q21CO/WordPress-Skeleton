...
class GFDropbox extends GFFeedAddOn {
	...
	public function init() {
		...
		if ( ! $this->get_plugin_setting( 'defaultAppEnabled' ) && $this->initialize_api() ) {
			...
			add_action( 'gform_editor_js_set_default_values', array( 'GF_Field_Dropbox', 'set_default_label' ) );
		}
	...
	public function scripts() {
		...
		$scripts = array(
			...
			array(
				...
				'version' => $this->_version,
				'enqueue' => array( array( $this, 'maybe_enqueue_plugin_settings_script' ) ),
			),
			array(
	...
	/**
	 * Check if Dropbox plugin settings script should be enqueued.
	 * 
	 * @access public
	 * @return bool
	 */
	public function maybe_enqueue_plugin_settings_script() {
		return rgget( 'page' ) == 'gf_settings' && rgget( 'subview' ) == 'gravityformsdropbox';
	}
	 ...
	 * 
	 * @access public
	 * @param $id - The ID of the feed requesting duplication.
	 * @return bool
	 */
	...
	public function is_valid_app_key_secret( $app_key = null, $app_secret = null ) {
		...
		return ! is_wp_error( $auth_request ) && $auth_request['response']['code'] == 200;

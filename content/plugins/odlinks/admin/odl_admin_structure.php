...
function process_odlinksstructure(){
  ...
  if (!$id) $id=0;
  ?>
  <h2>ODLinks Sturcture - Add/Edit Categories</h2>
  <?php
  ...
  if ($view) {
    ...
    echo '<div style="clear:both;"><p><a href="' . $PHP_SELF . '?page=odlinksstructure">back to main page</a></p></div>';
...
function odl_edit_category($id) {
  ...
  <div class="wrap">
    <h3 class="hndle">Add/Edit Categories</h3>
    <div class="inside">
      ...
      <P>
      <label>&nbsp;</label>
      <input type="submit" value="Save category">&nbsp;&nbsp;<input type=button value="Cancel" onclick="history.go(-1);"> 
      </form>
    </div>
    <div class="clear"></div>
  </div>
...
function odl_view_category($id) {
  ...
  <div class="wrap">
    ...
    <h3 class="hndle">Categories and Subcategories</h3>
    <div class="inside">
      <p><input type="button" value="Add Category" class="button-primary" onclick="document.location.href='<?php echo $PHP_SELF;?>?page=odlinksstructure&odlinks_admin_action=editCategory&c_id=0';"></p>

<p>View a list of all categories and the subcategories<br />
Here you can Add, Edit, or Delete categories<br>
To edit an existing category, click on the category name. This will display the categories Management page with fields populated for this category.</p>


      <p style="clear:both;text-align:right;padding:0 10px;"><img src="<?php echo get_bloginfo('wpurl'); ?>/wp-content/plugins/odlinks/images/delete.png"> delete category, including subcategories and all links within.</p>
...
function odl_main_cats($parent,$lev,$orderby,$how) { 
  ...
  $out = ""; 
  if($lev == 0){print "\n";} 
  if ( !isset($how) || !$how ){$how = "ASC";} 
  if ( !isset($orderby) || 
       ( isset($orderby) && $orderby <> "c_date") ) {$orderby = "c_title";} 
  $space = ""; 
  ...
  for ($i=0; $i<count($categories); $i++){
    ...
    <?php
    $nxtLev = $lev + 1;
    $print = odl_main_cats($id,$nxtLev,$orderby,$how); 
  } 

...
class Thumbshot{
  ...
  var $debug_IP = '';  // Enable debug for selected IP only
  ...
  var $original_image_size = 'S';  // XS, S, M, L (free) and XL, XXL, XXXL, XXXXL (paid)
  ...
  var $cache_days = 365;    // Regular images
  var $err_cache_days = 2;  // Service and error images
  var $queued_cache_days = 0;  // Images displayed on queue
  var $chmod_files = 0666;  // chmod created files
  var $chmod_dirs = 0777;    // chmod created directories
...
LvX4yglOQdGPf3juCHnJhcUJKmiOltGD2CyAAAMA9pVY15kNU24AAAAASUVORK5CYII=" alt="" title="Reload this thumbshot" /></a>';
  ...
  function get_thumbnail( $force = false )
  {
    ...
    if( $force || !file_exists($file) || @filemtime($file) <= $cutoff )
      ...
      if( $jpgurl = $this->get_remote_thumbnail() )
        ...
        $this->debug_disp( 'save temp image', $tmpfilename );
  ...
  function get_resized_thumbnail( $force = false )
  {
    ...
    {  // Debug
      ...
      $msg = 'Original image PATH: ' . filesize($file_orig);
  ...
  function check_dir()
  {
    ...
    if( !file_exists($file) )
    {
      ...
      if( !file_exists($file) )
      {
        $this->debug_disp('Unable to create <i>index.html</i> file!'. $file);
        return false;
      }
    }
    return true;
